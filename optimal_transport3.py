...
def run_example_triangles(l0,l1,l2,b0,b1,b2,vis = True):
	...
	if vis == True:
		...
		print(count)
		plt.scatter(l0[0,:], l0[1,:], color='green',s=b0*500, zorder=2)
		plt.scatter(l1[0,:], l1[1,:], color='blue',s=b0*500, zorder=2)
		plt.scatter(l2[0,:], l2[1,:], color='red',s=b0*500, zorder=2)
		plt.axis('off')
		plt.show()

	return T

def run_example_centers(l0,l1,l2,b0,b1,b2,vis = True):
	r = len(l0[0,:])
	s = len(l1[0,:])
	t = len(l2[0,:])
	b = np.concatenate([b0,b1,b2])
